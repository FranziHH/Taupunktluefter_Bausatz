 ...
 void show_Eingabe_Bildschirm()
{
  ...
  byte ZVS = 0;  
  if (ABC.length() >= 21) ZVS = 1; else ZVS = 0;
  byte z = ABC.length()-(ZVS*21); 

  //------------------------------------- Unterstrich blinkt an der Stelle, wo gerade editiert wird
  torgel = not torgel;
  if (torgel) { oled.invertText(false); OLED_print_char(' ', 5+ZVS , z);} 
    else      { oled.invertText(true) ; OLED_print_char(' ', 5+ZVS , z);} 
  oled.invertText(false);
  ...
  if (point.pos != old_point.pos)
  {
        ...
        { 
          ...
          oled.invertText(true); 
         if (Case) OLED_print_char( Tastatur_40[point.pos],point.x ,point.y); 
          else OLED_print_char( Tastatur_40_k[point.pos],point.x ,point.y);

          if (point.pos < 40) oled.invertText(false); 
          oled.setScale(4); 
          if (Case) OLED_print_char( Tastatur_40[point.pos],0,17); 
            else  OLED_print_char( Tastatur_40_k[point.pos],0,17); 
       ...
       if (last_Button_count != Button_count)
        { 
          ...
          if (point.pos < 41)  //------------- Buchstabe ist markiert ==> Buchstabe übernehmen
          {
            ...
            {
              if (ABC.length() >= 20)OLED_println("                    ",6,0); //  Zeile 6 freigeben
...
void Optionen_zu_Variablen()
{
  ...
  { //-------------------------------------------------------------- Sensorenkorrektur ---------------------------------------------------------
    ...
    if( StringVergleich(String(Parameter[i].Bezeichnung), "Charts"))      use_Charts = Bool(Parameter[i].Zeichenfolge);
...
bool Wartung(byte MenueItem)
{
  ...
  if(StringVergleich(String(Parameter[MenueItem].Bezeichnung), "Format LittleFS")) 
  {
