...
void Taupunkte_berechnen() {
  ...
  if (isnan(h1) || isnan(t1) || h1 > 100 || h1 < 1 || t1 < -40 || t1 > 80 || isnan(h2) || isnan(t2) || h2 > 100 || h2 < 1 || t2 < -40 || t2 > 80) {
    fehler = true;
  }
  ...
  if (Datenspeicher > 0) {
    if (chronik_update_all() == false) {
      Serial_Debugging_println("Chronik Update fehlgeschlagen");
    }
  }
  ...
  if ((t1 < TEMP1_min) or (t1 > TEMP1_max)) {
    rel = false;
  }
  if ((t2 < TEMP2_min) or (t2 > TEMP2_max)) {
    rel = false;
  }
  ...
  if (rel == true) {
    ...
    if (Luefter_Start == 0) {
      Luefter_Start = millis();
    }
  } else if ((millis() - Luefter_Start) > (Luefter_Mindestlaufzeit * 1000 * 60)) // Mindestlaufzeit in Minuten
  {
    digitalWrite(RELAIPIN, RELAIS_AUS); // Relais ausschalten
    if (Luefter_Start > 0) {
      Luefter_Laufzeit = Luefter_Laufzeit + (millis() - Luefter_Start); // Lüfterlaufzeit in Millisekunden
      Luefter_Start = 0;
    }
  }
  //------------------------------------------------ Ampel schalten ----------------------------------
  if (use_Ampel == true) {
    if (DeltaTP > SCHALTmin) {
      set_Ampel_2(1, 100); // grün
    }
    if ((DeltaTP < SCHALTmin) and (DeltaTP > 0)) {
      set_Ampel_2(2, 100); // gelb
    }
    if (DeltaTP <= 0) {
      set_Ampel_2(3, 100); // rot
    }
    if ((t1 < TEMP1_min) or (t1 > TEMP1_max)) {
      set_Ampel_2(3, 100); // rot
    }
