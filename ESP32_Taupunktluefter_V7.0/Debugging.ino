...
void Serial_Debugging_println(String f) {
  if (debugging == true) {
    Serial.println(f);
  }
}
...
void Flash_available() { Serial.println("Momentan sind " + String(ESP.getFreeHeap()) + " Byte Flash-Speicher verfügbar"); }
...
void serial_WIFI_Status() {
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("WIFI ist CONNECTED");
  } else {
    Serial.println("WIFI ist DISCONNECTED");
  }
}
