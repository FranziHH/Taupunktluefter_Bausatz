...
String trim_length_datastream(String Datenstream, int laenge_pro_Datensatz)
{
 ...
 while (Anzahl_Werte > ( Datenspeicher - 1))
 {
   Datenstream.remove(0,1);
    ...
    if (Datenstream.substring(0,1)== ",") Datenstream.remove(0,1); // verbleibendes Komma am am Anfang des Datenstreams entfernen
...
bool test_TimeDate() //--------------------------------------- Datum und Zeit  testen für Chart labels
{
  ...
  // Tag
   i = stamp.substring(0,2).toInt();   if ((i < 1)  or (i >31)) return false;
   c = stamp.charAt(2); if ( c != '.')return false;

    // Monat
   i = stamp.substring(3,5).toInt(); if ((i < 1)  or (i >12)) return false;
   c = stamp.charAt(5); if ( c != '.')return false;

  //Jahr
   i = stamp.substring(6,8).toInt();  if ((i < 26)  or (i > 99)) return false;
   c = stamp.charAt(8); if ( c != ' ')return false;

  // Stunde
  i = stamp.substring(9,11).toInt(); if ((i < 0)  or (i > 23)) return false;
   c = stamp.charAt(11); if ( c != ':')return false;

   //Minute   
  i = stamp.substring(12,14).toInt(); 
   if ((i < 0)  or (i > 59)) return false;
   
  tmp_Chart_TimeDate = Chart_TimeDate;
  if (tmp_Chart_TimeDate.length() < 16) tmp_Chart_TimeDate = ""; else tmp_Chart_TimeDate += ","; 
...
bool test_temp_in() //--------------------------------------- Innentemperatur testen 
{
  ...
  if ( isnan(t1) or ( t1 < -50 ) or ( t1 > 60)) return false;                                 // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_temp_in = Chart_temp_in;                                                          // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if ( tmp_Chart_temp_in.length() < 7) tmp_Chart_temp_in = ""; else tmp_Chart_temp_in += ","; // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_temp_Out() //--------------------------------------- Außentemperatur testen 
{
 char buffer[6];
  Watchdog_reset();                                                                               // Watchdog zurücksetzen
   if ( isnan(t2) or ( t2 < -50 ) or ( t2 > 60)) return false;                                    // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_temp_out = Chart_temp_out;                                                            // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if (tmp_Chart_temp_out.length() < 7) tmp_Chart_temp_out = ""; else tmp_Chart_temp_out += ",";   // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_air_in() //--------------------------------------- Innen-Luftfeuchte testen 
{
 char buffer[6];
  Watchdog_reset();                                                                           // Watchdog zurücksetzen  
   if ( isnan(h1) or ( h1 < 0 ) or ( h1 > 100)) return false;                                  // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_air_in = Chart_air_in;                                                            // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if ( tmp_Chart_air_in.length() < 7) tmp_Chart_air_in = ""; else tmp_Chart_air_in += ",";    // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_air_out() //--------------------------------------- Außen-Luftfeuchte testen 
{
 char buffer[6];
  Watchdog_reset();                                                                           // Watchdog zurücksetzen  
   if ( isnan(h2) or ( h2 < 0 ) or ( h2 > 100)) return false;                                  // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_air_out = Chart_air_out;                                                          // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if ( tmp_Chart_air_out.length() < 7) tmp_Chart_air_out = ""; else tmp_Chart_air_out += ","; // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_delta_TP() //--------------------------------------- Unterschied der Taupunkte 1/2 testen
{
  char buffer[6];
  Watchdog_reset();                                                                             // Watchdog zurücksetzen  
   if (isnan(DeltaTP) or ( DeltaTP < -50 ) or ( DeltaTP > 60)) return false;                    // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_tp_delta = Chart_tp_delta;                                                          // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if (tmp_Chart_tp_delta.length() < 7) tmp_Chart_tp_delta = ""; else tmp_Chart_tp_delta += ","; // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_Taupunkt_1() //--------------------------------------- Taupunkt 1  testen
{
 char buffer[6];
  Watchdog_reset();                                                                         // Watchdog zurücksetzen  
   if (isnan(Taupunkt_1) or ( Taupunkt_1 < -50 ) or ( Taupunkt_1 > 60)) return false;       // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_tp_in = Chart_tp_in;                                                            // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if (tmp_Chart_tp_in.length() < 7) tmp_Chart_tp_in = ""; else tmp_Chart_tp_in += ",";      // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_Taupunkt_2() //--------------------------------------- Taupunkt 2 testen
{
 char buffer[6];
  Watchdog_reset();                                                                         // Watchdog zurücksetzen  
   if (isnan(Taupunkt_2) or ( Taupunkt_2 < -50 ) or ( Taupunkt_2 > 60)) return false;       // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_tp_out = Chart_tp_out;                                                          // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if (tmp_Chart_tp_out.length() < 7) tmp_Chart_tp_out = ""; else tmp_Chart_tp_out += ",";   // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
...
bool test_lz() //--------------------------------------- Lüfterzeit testen
{
 ...
 int LZ_in_Min = Luefter_Laufzeit / 60000;
  Watchdog_reset();                                                                         // Watchdog zurücksetzen  
  if (isnan(LZ_in_Min) or (LZ_in_Min < 0) or (LZ_in_Min > chronik_interval)) return false;  // Testen, ob der Wert gültig sein kann, wenn nicht, wird die Funktion abgebrochen und false zurückgegeben
  tmp_Chart_lz = Chart_lz;                                                                  // Stream sicherheitshalber ersteinmal einem temporär-Streem zuweisen
  if (tmp_Chart_lz.length() < 7) tmp_Chart_lz = ""; else tmp_Chart_lz += ",";               // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
  ...
  Luefter_Laufzeit = 0; 
  if (Luefter_Start > 0) Luefter_Start = millis();                                         // Lüfter Laufzeit zurücksetzen
  return true;                                                                              // Rückgabewert: alles gut gelaufen
}//-----------------------------------------------------------------------------------------
  ...
  bool test_lzR() //--------------------------------------- Radon Lüfterzeit testen
{
 ...
 int LZ_in_Min = Luefter_Laufzeit_Radon / 60000;
  ...
  if (tmp_Chart_lzR.length() < 7) tmp_Chart_lzR = ""; else tmp_Chart_lzR += ",";               // Ist schon ein Wert vorhanden dann "," einfügen, sonst Datenstream komplett leeren
  ...
  if (Radonsignal == true) Luefter_Radon_Start = millis();                                         // Lüfter Laufzeit zurücksetzen
  return true;                                                                             // Rückgabewert: alles gut gelaufen
...
{   
  ...
  {
    Serial_Debugging_println(F("An Error has occurred while mounting LittleFS")); Fehler_speichern(120);
    return false;
  }
    
 File file = LittleFS.open(fileDateTime);
  if(!file){
   Serial_Debugging_println(F("Failed to open file for reading: fileDateTime"));  Fehler_speichern(121);
   ...
   OLED_update();
   while(file.available()){  buffer = buffer + char(file.read()); }
  file.close();
...
{   
  ...
  if (!LittleFS.begin()) 
  {
    Serial_Debugging_println(F("An Error has occurred while mounting LittleFS")); Fehler_speichern(122);
    return "";
  }
 File file = LittleFS.open(name);
 if ((file.size()) < 7) { Fehler_speichern(130); return "";}
  if(!file){
   Serial_Debugging_println("Failed to open file for reading: "+name); Fehler_speichern(123);
   OLED_Zeile_loeschen(3);
...
{ 
 ...
 if (!LittleFS.begin()) {
    Serial_Debugging_println("An Error has occurred while mounting LittleFS"); Fehler_speichern(124);
    OLED_Zeile_loeschen(3);
...
if (file.print(daten)) {
    ...
    Serial_Debugging_print("Fehler beim Schreiben der Datei "+name+" in den LittleFS"); Fehler_speichern(125);
    OLED_Zeile_loeschen(3);
   OLED_println("Fehler: "+name, 3, 0);
   OLED_update();
...
{
  if (!LittleFS.format()){ Fehler_speichern(129); return false;}; 
...
{ 
    ...
    if(formatted){
      ...
      Serial_Debugging_println("\n\nError formatting"); Fehler_speichern(126);
    }
...
bool load_Chronik_from_LittleFS()
{
  ...
  if (!LittleFS.begin()) 
  {
    Serial_Debugging_println(F("An Error has occurred while mounting LittleFS"));  Fehler_speichern(127);
    return false;
  }
  ...
  if( Datenspeicher > 0 ) // Datenspeicherung findet nur statt, wenn die Konstante "Datenspeicherung" mit einer Zahl > 0 belegt ist
  {
     ...
     if (HTML_processor_is_working == false)              // Überschneidungen mit dem Server zu verhindern
    {
      if (last_save + chronik_interval < rtc.getEpoch())  // wie lange ist die letzte Datenspeicherung her?
      {
        ...
        if (test_TimeDate()== false)   { Fehler_speichern(500); return false;} //--------------------------------------- Datum und Zeit testen 
        if (test_temp_in()== false)    { Fehler_speichern(501); return false;} //--------------------------------------- Innentemperatur testen 
        if (test_temp_Out()== false)   { Fehler_speichern(502); return false;} //--------------------------------------- Außentemperatur testen 
        if (test_air_in()== false)     { Fehler_speichern(503); return false;} //--------------------------------------- Innen-Luftfeuchte testen 
        if (test_air_out()== false)    { Fehler_speichern(504); return false;} //--------------------------------------- Außen-Luftfeuchte testen 
        if (test_delta_TP()== false)   { Fehler_speichern(505); return false;} //--------------------------------------- Unterschied der Taupunkte testen
        if (test_Taupunkt_1()== false) { Fehler_speichern(506); return false;} //--------------------------------------- Taupunkt 1  testen
        if (test_Taupunkt_2()== false) { Fehler_speichern(507); return false;} //--------------------------------------- Taupunkt 2 testen
        if (test_lz() == false)        { Fehler_speichern(508); return false;} //--------------------------------- ------Lüfterzeit testen
        if (test_lzR() == false)        { Fehler_speichern(509); return false;} //-------------------------------------- Radon Lüfterzeit testen
    ...
    } else Serial_Debugging_println(F("HTML_processor_is_working"));
  }
  return true;
} 
//-------------------------------------------------------------------------------------------------------------------------
...
{  
  int z = 0;
   z = (Chart_TimeDate.length()+1) / Chart_TimeDate_length ;Serial.println(String(z)+ " - Chart_TimeDate: " + Chart_TimeDate);  
   z = (Chart_temp_in.length()+1) / laenge_Datensatz ;Serial.println(String(z)+ " - Chart_temp_in: " + Chart_temp_in);  
   z = (Chart_air_in.length()+1)  / laenge_Datensatz;Serial.println(String(z)+ " - Chart_air_in:  " + Chart_air_in);  
   z = (Chart_tp_in.length()+1)   / laenge_Datensatz;Serial.println(String(z)+ " - Chart_tp_in:   " + Chart_tp_in);  
  
   z = (Chart_temp_out.length()+1)/ laenge_Datensatz;Serial.println(String(z)+ " - Chart_temp_out:" + Chart_temp_out);  
   z = (Chart_air_out.length()+1) / laenge_Datensatz;Serial.println(String(z)+ " - Chart_air_out: " + Chart_air_out);    
   z = (Chart_tp_out.length()+1)  / laenge_Datensatz;Serial.println(String(z)+ " - Chart_tp_out:  " + Chart_tp_out);
  
   z = (Chart_tp_delta.length()+1)/ laenge_Datensatz;Serial.println(String(z)+ " - Chart_tp_delta:" + Chart_tp_delta);
   z = (Chart_lz.length()+1) /  	  laenge_Datensatz;Serial.println(String(z)+ " - Chart_lz:" + Chart_lz);
   z = (Chart_lzR.length()+1) /     laenge_Datensatz;Serial.println(String(z)+ " - Chart_lzR:" + Chart_lzR);
